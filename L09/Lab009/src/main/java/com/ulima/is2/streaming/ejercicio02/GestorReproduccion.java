...
public class GestorReproduccion {
    ...
    public String iniciarReproduccion(String idUsuario, String idVideo, double anchoBandaMbps) {
        if (!servicioSuscripcion.cuentaEstaActiva(idUsuario)) {
            throw new IllegalArgumentException("ERROR: Cuenta inactiva o suspendida");

        }

        if (!servicioCDN.cdnEstaDisponible()) {
            throw new IllegalArgumentException("ERROR: Servicio CDN no disponible temporalmente");
        }
        ...
        if (url == null || url.isEmpty()) {
            throw new IllegalArgumentException("ERROR: Video no encontrado en calidad " + calidad);
        }
